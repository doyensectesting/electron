...
const handleReturnValue = function (event, code) {
  try {
    event.returnValue = [null, code()]
  } catch (error) {
    event.returnValue = [errorUtils.serialize]
  }
}

ipcMain.on('ELECTRON_BROWSER_GET_LAST_WEB_PREFERENCES', function (event) {
  handleReturnValue(event, () => event.sender.getLastWebPreferences())
})

ipcMain.on('ELECTRON_BROWSER_CLIPBOARD_READ_FIND_TEXT', function (event) {
  handleReturnValue(event, () => electron.clipboard.readFindText())
})

ipcMain.on('ELECTRON_BROWSER_CLIPBOARD_WRITE_FIND_TEXT', function (event, text) {
  handleReturnValue(event, () => electron.clipboard.writeFindText(text))
})
